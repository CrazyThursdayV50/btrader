...
impl bTrader {
    // Constructor
    pub async fn new(config_path: &str) -> bTrader {
        ...
        let general: General = tokio::task::spawn_blocking(|| Binance::new(None, None))
            .await
        ...
        let result = tokio::task::spawn_blocking(move || match general.exchange_info() {
            Ok(answer) => answer,
        ...
        let depth_cache = DepthCache::new(&socket_pairs, 8, 1).await;
        let calculation_cluster = CalculationCluster::new(relationships, depth_cache, config).await;
        bTrader {
            calculation_cluster,
        }
    }
