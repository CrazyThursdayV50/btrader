...
impl DepthCache {
    ...
    pub async fn new(
        ...
        let queue: Arc<RwLock<VecDeque<DepthOrderBookEvent>>> =
        ...
        // Initialize stuff
        let market: Market = task::spawn_blocking(|| Binance::new(None, None))
            .await
        ...
        for symbol in &symbols {
            let order_book: OrderBook = task::spawn_blocking({
                let market = market.clone();
                let symbol = symbol.clone();
                move || get_snapshot(&market, &symbol)
            })
            .await
            .expect("Failed to get order book snapshot");
                ...
                loop {
                    ...
                    if let Some(event) = q.pop_front() {
                        let mut this_map = match map_clone.write() {
                            Ok(m) => m,
                            Err(e) => {
                                panic!("Failed to get map for write while processing queue: {}", e)
                            }
                        };
