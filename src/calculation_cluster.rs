...
impl CalculationCluster {
    pub async fn new(
        ...
        let account: Account = tokio::task::spawn_blocking(move || {
            Binance::new(Some(config_clone.api_key), Some(config_clone.api_secret))
        })
        .await
        .expect("Failed to create Binance account");
        let config_clone = config.clone();
    ...
    pub async fn start(&self) {
        ...
        if self.config.telegram_enabled {
            ...
            tokio::spawn(async move {
                bot.send_message("btrader start".to_string()).await;
            })
            .await
            ...
            || self.config.trading_execution_cap == -1
        {
            for rel in &relationships_names {
                ...
                if (deal.get_profit() >= (self.config.trading_profit_threshold / 100.0))
                    && ((self.get_epoch_ms() - deal.get_timestamp())
                        <= self.config.trading_age_threshold)
                {
                    println!(
                        "[{}] Deal: {:?}...",
                    ...
                    if self.config.telegram_enabled {
                        ...
                        tokio::spawn(async move {
                            bot.send_message(format!(
                                "[{:+.3}%] Deal: {:?}...",
                                deal_clone.get_profit() * 100.0,
                                deal_clone.get_actions()
                            ))
                            .await;
                        });
                    }
                    if self.config.trading_enabled {
                        ...
                        let bot = self.bot.clone();
                        tokio::spawn(async move {
                            bot.send_message("Deal executed.".to_string()).await;
                        });
                        execution_count += 1;
                    } else {
                        println!(
                        ...
                        if self.config.telegram_enabled {
                            ...
                            tokio::spawn(async move {
                                bot.send_message(
                                    "[INFO] Trading is not enabled, skipping...".to_string(),
                                )
                                .await;
                            });
                        }
            ...
            for (j, pair_name) in pair_names.iter().enumerate() {
                ...
                if pair_actions[j] == "BUY" {
                    ...
                    for ask in prices.iter() {
                        tmp_quantity =
                            self.correct_quantity(helper_quantity / ask.price, pairs[j].get_step());
                    ...
                    for bid in prices.iter() {
                        if bid.qty >= helper_quantity {
                            current_quantity += self
                                .correct_quantity(helper_quantity, pairs[j].get_step())
                                * bid.price;
                        } else {
                            current_quantity +=
                                self.correct_quantity(bid.qty, pairs[j].get_step()) * bid.price;
                        }
